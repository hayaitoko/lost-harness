...
impl Tool for FetchUrlTool {
    ...
    fn destination(&self, args: &serde_json::Value) -> Option<String> {
        ...
        url::Url::parse(raw.trim()).ok()?.host_str().map(str::to_string)
    }
...
async fn fetch_readable(raw: &str) -> Result<serde_json::Value, String> {
    let mut current =
        url::Url::parse(raw).map_err(|e| format!("not a valid URL: {e}"))?;

    // A dedicated client per call, redirects OFF (we follow manually so the
    // SSRF re-check runs — with DNS — on every hop).
    ...
    let resp = loop {
        ...
        let resp = client
            ...
            .send()
            .await
            .map_err(|e| format!("request to {} failed: {e}", current.host_str().unwrap_or("?")))?;
...
async fn ssrf_check(url: &url::Url) -> Result<(), String> {
    match url.scheme() {
        "http" | "https" => {}
    ...
    if is_private_endpoint(url.as_str()) {
        return Err(format!("refusing to fetch a private/internal address ({host})"));
    }
...
pub(crate) fn html_to_text(body: &str) -> (Option<String>, String) {
    ...
    let block_broken = no_scripts.replace("</p>", "\n").replace("<br>", "\n").replace("<br/>", "\n").replace("<br />", "\n").replace("</div>", "\n").replace("</li>", "\n").replace("</h1>", "\n").replace("</h2>", "\n").replace("</h3>", "\n");
...
mod tests {
    ...
    fn blocks_every_internal_ip_family() {
        ...
        assert!(!is_blocked_ip(IpAddr::V6(Ipv6Addr::new(0x2606, 0x2800, 0, 0, 0, 0, 0, 1))));
    }
    ...
    async fn run_refuses_non_http_schemes_and_private_hosts_without_network() {
        ...
        for bad in [
            ...
            let out = tool
                .run(ToolInput::new(json!({ "url": bad })), &ctx)
                .await;
