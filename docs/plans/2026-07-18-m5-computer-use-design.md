...
*This section supersedes §2.3.2 (fingerprint), §2.3.1 (reversibility policy wiring), and the routing half of §2.5, and re-states the affected build slices. Everything else in the design stands. The review is correct on all three mechanics; the fixes below reuse the real spine — no parallel machinery, no fingerprint-recompute, no `HookResult::Modify`.*

The root cause of all three gaps is one modelling error: the original design tried to make the tool's **args** be pixels/coordinates and then *rewrite* them into a semantic target inside the hook chain. Every downstream problem (Modify-vs-Ask, fingerprint-before-hooks, covers()-divergence, the too-many-rounds loop) follows from that rewrite. Delete the rewrite and the machinery already does what we need.

### Fix 1 — The semantic target lives in the args from the start; the fingerprint is already semantic (gap #1)

**The change:** act tools take a **semantic locator as their args**, never pixels. The model calls:

```
ui_click{ app:"Mail", role:"button", label:"Reply" }        // NOT ui_click{x:450,y:320}
```

Pixels are computed *inside* `Tool::run` at synthesis time (resolve locator → element → point), and never appear in the tool's args at all. `SynthAction::Click(Point)` (§2.2) is the backend's *private* type, produced after re-resolution; it is not the tool's arg shape. This resolves the §2.1-vs-§2.3 arg inconsistency the review flagged: the arg shape is the semantic locator, full stop.

**Why this dissolves gap #1 mechanically** — walking the review's four bullets against the real code:

- `ActionFingerprint::of(&call.name, &call.args)` at `dispatch.rs:460` hashes the *original* args, before the chain. That is now **exactly right**, because the original args *are* the semantic target. `of("ui_click", {app,role,label})` is a stable semantic fingerprint. A click on a different control has different args → different fingerprint → re-prompt. This is the "un-driftable" binding — delivered by the fingerprint that already exists, with no recompute.
- **No `HookResult::Modify` anywhere.** The hook never rewrites `ctx.input`. So the "Modify and Ask are mutually exclusive" problem (`hooks/mod.rs:428` vs `:432`) never arises, and the per-round context rebuild from `call.args` (`dispatch.rs:471-472`) is harmless — every round recomputes the *same* semantic context.
- **`covers()` now aligns by construction.** `PermissionHook` computes `ActionFingerprint::from_ctx(ctx)` from `ctx.input.args` (`permission.rs:411`). Because the hook does not mutate `ctx.input`, that fingerprint is **byte-identical** to the `dispatch.rs:460` fingerprint the grant was recorded under. They diverged in the old design *only because* it Modified. No Modify → they are the same hash → a recorded grant satisfies the check → the approval loop settles in 2 passes, never hitting `MAX_APPROVAL_ROUNDS` (`dispatch.rs:467`).
- **The dialog string comes free from `canonical`.** `ApprovalRequest.command` is `canonical` = `format!("{} {}", call.name, call.args)` (`dispatch.rs:457,560`). With semantic args, that renders as `ui_click {app:"Mail",role:"button",label:"Reply"}` — human-vettable as-is. No `match_text` override is needed for the dialog (the frontend formats "click **Reply** in Mail" from the structured fields — see Fix 2 / slice 4). `match_text` (`tools/mod.rs:342`) is overridden only for the denylist/pattern surface (`command_text`, `dispatch.rs:479`), which is a separate concern.

**Node-id stability (the review's minor point), folded in:** re-resolution is by the **semantic tuple `(app, role, label, geometry)`**, never by an opaque handle. macOS `AXUIElement` refs, AT-SPI paths, and UIA `RuntimeId`s are not reliably stable across snapshots, so `node_id` is a *within-snapshot* convenience only and never a cross-snapshot key or a fingerprint input. `ActionTarget` (§2.2) keeps `node_id` for one-shot use inside a single snapshot; the fingerprint and the re-verify both key off the semantic tuple. This is why the fingerprint is stable enough to bind a session grant to.

### Fix 2 — Reversibility maps onto tool RiskClass + a `covers_once` floor; it does not fight the binary policy (gap #2)

The review is right: `lib.rs:579-585` sets every non-`Safe` tool to whole-tool `Ask`, `Continue`/`Allow` don't short-circuit (`hooks/mod.rs:427`), so control always reaches `PermissionHook`, and a "Reversible → Continue" upstream hook cannot suppress the Ask. The original "all act tools Dangerous, hook returns Continue for reversible" wiring is unbuildable. Correct mapping — the three reversibility tiers land on **static tool boundaries + one per-target floor**, all of which the real dispatcher already supports:

| Tier (per §2.3.1) | Tools | Static `RiskClass` | Effect via the REAL policy path |
|---|---|---|---|
| **Reversible** (no side effect / pure navigation) | `capture_screen`, `read_ui_tree`, `read_clipboard`, `ui_scroll`, (opt.) hover/move/focus | **`Safe`** | `lib.rs:580-583` → whole-tool `Allow` + pre-trusted → **no prompt**. Honest: these don't actuate a discrete control (scroll is intrinsically reversible; it has no "sometimes irreversible" mode, so a static class is correct). |
| **Consequential** (click/keypress/drag on a normal control) | `ui_click`, `ui_type`, `ui_key`, `ui_drag` — *default* | **`External`** | `lib.rs:584-585` → whole-tool `Ask`. `resolve_grant(External, Session\|Always)` → `(Session, Fingerprint)` (`approval.rs:251-254`) — **fingerprint-pinned session grant, never whole-tool**. The fingerprint is the semantic target (Fix 1), so this *is* "Session-grantable per semantic target," delivered by the real matrix. |
| **Irreversible** (Send/delete/buy/pay/… verb set, or sensitive app) | same tools, when the resolved target matches the fail-safe set | **`External` tool + `covers_once` floor** | `OnScreenActionHook` consults `ledger.covers_once(&fp)` (`approval.rs:171`), **not** `covers()`, exactly like `ProtectedPathHook` (`hooks/mod.rs:51-56`). A `Session` grant is invisible to `covers_once`, so an irreversible target is **structurally un-coverable by any standing grant** and re-Asks every time — Once-only, human-present. |

**Why `External`, not `Dangerous`, for the act tools.** `Dangerous` collapses *every* answer to `(Once, fp)` (`approval.rs:246`) — which forbids the very per-target session grant the Consequential tier needs. `External` is both the *correct* class (a `ui_click` on **Send** reaches beyond the machine — `RiskClass::External` doc, `tools/mod.rs:257`) **and** the one that permits a fingerprint-pinned (never whole-tool, `approval.rs:252`, and never persistable — `persist_rule_allowed` is `Write`-only, `approval.rs:271`) session grant. The irreversible **property** the task says to preserve (Once-only, no standing grant, double re-snapshot) is preserved *exactly* — we simply move its **mechanism** from "static `Dangerous` risk" to "a `covers_once` floor," because the static-risk mechanism cannot coexist with per-target session grants inside one tool, whereas the `covers_once` floor can (it is orthogonal to the tool's risk class and is already a tested pattern). Net: irreversible is exactly as locked; consequential gains a sound, spine-native session grant.

**`OnScreenActionHook` (revised), a `GatingHook` holding the shared `Arc<ApprovalLedger>`, registered after the Sandbox/ProtectedPath floors and before `PermissionHook`** (the `build_pretooluse_chain_full` slot, `hooks/mod.rs:540-551`). Per pass it re-resolves the locator against a *fresh* snapshot, classifies reversibility, and:

- **Irreversible target** → `covers_once(&fp)` ? `Continue` : `Ask(verb+target)`. This is the floor; a standing grant can never satisfy it (`approval.rs:171`).
- **Consequential target** → `Continue` (defer to `PermissionHook`, which does the whole-tool `Ask` unless `covers(&fp)` — the real Session-grant path).
- **Reversible target reached by an act tool** (rare — the fail-safe default is Consequential, §2.3.1) → not used by the `Safe` tools, which never enter this hook's gate at all; kept only as the explicit fail-safe branch that widens *toward* Ask, never away from it.
- It does **not** mutate `ctx.input` (Fix 1). It annotates a new out-field `ctx.on_screen = { semantic_target, verb, app, reversibility, thumbnail_region }` for the dialog (slice 4), following the `ctx.routing` annotation precedent (`hooks/mod.rs:215-216`) — the hook writes, the dispatcher reads.

**One small, precedented dispatcher edit (not a rewrite).** For an irreversible Ask, the slice-4 dialog hides Session/Always, so the user answers `Once` → `(Once, fp)` → `covers_once` true on the re-run → settles with zero dispatcher change. To make a *misbehaving/bypassed* client fail **gracefully** rather than fail-closed-by-looping, extend the existing protected-path Once-pin (`dispatch.rs:611`) from `by == "protected_path"` to also fire for `by == "computer_action"`:

```rust
if (by == "protected_path" || by == "computer_action") && scope != GrantScope::Once {
    self.ledger.grant(GrantTarget::Fingerprint(fingerprint.clone()), GrantScope::Once);
}
```

This is the *same* mechanism `ProtectedPathHook` already relies on — a one-line additive change, not a fingerprint-path or loop rewrite. **The invariant holds even without it** (a bypassed Session answer on an irreversible target yields a `(Session, fp)` grant that `covers_once` ignores → re-Ask → deny-by-rounds — fail-safe), so this edit is a UX robustness nicety, not a load-bearing security dependency.

**OQ-3 hardened structurally (new requirement).** Because the `Session`/`fp` grant lives in the **shared** ledger (`dispatch.rs:236` clones the `Arc` into every sub-dispatcher), a delegated/background helper on the app body — which *does* carry `Capability::ComputerUse` — could otherwise satisfy `covers()` for a target the main agent got a human to session-approve, and click it **unattended**. Close this the same way headless closes it: **exclude every `ui_*` act tool from every delegated persona's belt**, so `restricted_to` (`tools/mod.rs:386`) makes them physically absent (Unknown before any gate) in any sub-dispatcher. Act tools are foreground-attended-only, enforced by registry contents, not by a checkable flag. (Reversible `Safe` reads like `capture_screen` may remain in a helper belt — reading pixels is not actuation and is still routed local by Fix 3.)

### Fix 3 — Screenshot → local is two coupled guards at the layers that actually own routing (gap #3)

The review is right that `enforce_local_routing` is turn/model-selection-level (`loop_mod.rs:1758`), the gate classifies **text** (`gate.rs:125-138`), and the `ctx.routing` annotation only guards `capture_screen`'s **own immediate next hop** (`dispatch.rs:505-519`). One annotation cannot keep *subsequent* turns local while a frame lingers in the model-facing window. Two explicit couplings, both reusing `enforce_local_routing` and `ctx.routing` — no new routing surface:

1. **Producing hop (this turn).** `OnScreenActionHook` sets `ctx.routing = LocalRequired{reason:"a screenshot can contain anything on screen"}` for the pixel-producing tools (`capture_screen`, and `read_ui_tree`/`read_clipboard` when they carry a thumbnail/pixels) — exactly as `PrivacyFilterHook` annotates routing (`hooks/mod.rs:215-216`). The dispatcher's existing check (`dispatch.rs:505`) then returns `NeedsLocalReroute`, and `resolve_turn_outcome` (`loop_mod.rs:1758`) switches the rest of *this* turn local **before** the frame is ever assembled into a cloud-bound window. This is why capturing on a cloud turn does not leak: the capture itself is local (no egress), and the first hop that *would* send the frame is already rerouted.

2. **Lingering frames (subsequent turns/rounds).** Add an explicit coupling in `stream_to_provider`, tied to the **same eviction bookkeeping** §2.5 introduces: before each round's stream (the loop at `loop_mod.rs:1525-1558`) and at the initial gate (`loop_mod.rs:324`), if the model-facing window still contains ≥1 `ImageBlock` **and** no cloud-vision consent (OQ-1) is present, force `RoutingRequirement::LocalRequired` and select the endpoint via `enforce_local_routing` — failing loud if no local provider exists, identical to the tool-reroute no-candidate path. When the eviction policy drops the last frame, the coupling releases and `Auto` text-classification resumes. Concretely, a helper `routing_for_turn(binding, text_decision, image_in_window, cloud_vision_consent) -> RoutingRequirement` returns `LocalRequired` whenever `image_in_window && !consent`, else maps the text gate decision; it takes precedence over `Binding::Public`'s `Allow` (so "maximally private screenshot" holds above the binding until the OQ-1 consent toggle ships) and is a no-op once no frame remains.

Guard 1 handles "the turn that takes the shot," guard 2 handles "every turn while the shot is still in context." Together they make `LocalRequired` actually hold for the window §2.5 deliberately keeps 1-2 frames in — the coupling the review said was missing.

### Invariant M5 (restated with corrected mechanisms)

> An on-screen action's authorization binds to a **semantic target** (`app, role, label, geometry`, re-verified against a *fresh* snapshot at synthesis time), carried **in the tool's args** so the existing `ActionFingerprint` already pins it — never to raw coordinates, never via arg-rewriting.
> **Irreversible** actuations are gated by a **`covers_once` floor** (`approval.rs:171`, the `ProtectedPathHook` mechanism): Once-only, human-present, structurally un-coverable by any standing grant.
> **Consequential** actuations are `External`-risk and **session-grantable per semantic fingerprint** (`resolve_grant`, `approval.rs:251`) — never whole-tool, never persisted.
> Screenshots hold every turn they touch to `LocalRequired` via two coupled guards at the routing layers (Fix 3). Act tools are foreground-only by **registry absence** from headless *and* delegated bodies.

Legs, re-audited: **un-driftable** = the semantic-args fingerprint (Fix 1) ✅ now sound. **fail-safe / no-standing-grant for irreversible** = `covers_once` floor ✅ (mechanism moved, property identical). **un-stale** = double re-snapshot in the hook and again in `Tool::run` ✅ unchanged. **un-headless / un-unattended** = `ComputerUse` absent from `headless_server_default` (`tools/mod.rs:120`) *plus* `ui_*` excluded from delegated belts ✅ hardened.

### Revised build slices (each gate honestly achievable; integration-harness gates flagged)

- **Slice 0 — Multimodal wire format.** Unchanged. *Gate:* synthetic image round-trips to a vision seat; text-only seat degrades. **`cargo test --lib`.**
- **Slice 1 — Backend trait + read-only capture/AX (macOS), as `Safe` tools.** *Gate (lib):* tools absent under `--no-default-features` and in a headless `BodyEnv` — capability-filter unit test (`tools/mod.rs:423`), **`cargo test --lib`**. *Gate (on-machine):* both tools return a real frame + AX tree scoped to the foreground app — **needs a macOS integration/smoke run, not `cargo test --lib`** (there is no real screen in CI).
- **Slice 2 — Untrusted-source discipline + privacy/budget.** *Gate:* injected `[END UNTRUSTED…]`/tool-call string in screen text can't break the guard (mirror `calling.rs` guard test); `routing_for_turn` returns `LocalRequired` when `image_in_window` (unit-test the helper + `enforce_local_routing` with a synthetic image-present flag); per-image token cost + oldest-frame eviction unit test. **All `cargo test --lib`.**
- **Slice 3 — Input synthesis + reversibility model (REVISED).** `ui_scroll` = `Safe`; `ui_click/type/key/drag` = `External`; `OnScreenActionHook` (semantic-args fingerprint, `covers_once` irreversible floor, defer-to-`PermissionHook` for consequential); double re-snapshot in `Tool::run`. *Gates, all `cargo test --lib` against a **mock `ComputerBackend`** driven through the real `build_pretooluse_chain_full` + `ToolDispatcher`:* (a) a `ui_scroll` (Safe) dispatches `Ok` with **no** Ask; (b) a click whose mock tree resolves to a **Send** button hits the `covers_once` floor → Ask, and no standing grant is ever recordable for it (assert `covers_once` stays false after a Session answer); (c) a click on **Reply** approved `Session` records `(Session, FP_reply)`, a second identical call does **not** prompt, a different-label click **does**; (d) a target whose mock geometry changes between snapshots is refused (`Tool::run` → `Err`), not mis-clicked. Real OS synthesis is slices 5-6.
- **Slice 4 — Distinct approval UX.** `ctx.on_screen` annotation → copied into an `ApprovalRequest` on-screen variant `{semantic_target, verb, app, reversibility, thumbnail_region}` (dispatcher reads the annotation, `ctx.routing` precedent). *Gate (lib):* a standing grant can never cover an irreversible target (assert on the `covers_once` floor); a Session grant pins the semantic fingerprint, not pixels (assert on `ActionFingerprint::of` over the semantic args) — **`cargo test --lib`.** *Gate (UI):* the dialog renders the target thumbnail + verb and hides Session/Always for irreversible — **frontend/visual check, not `cargo test --lib`.**
- **Slice 5 — Windows + Linux backends + permission flows.** Unchanged in substance. *Gate:* each OS drives a real click end-to-end; a denied permission and a Windows UIPI/elevation mismatch each produce a typed error, never a silent no-op — **per-OS integration/smoke harness on real hardware, not `cargo test --lib`.**
- **Slice 6 — Feature/local-first hardening + wiring, incl. the two small dispatcher edits (the `computer_action` Once-pin at `dispatch.rs:611` and the `ctx.on_screen` read) and the delegated-belt `ui_*` exclusion.** *Gate:* `cargo build --no-default-features` green with zero core-graphics/windows/x11 deps (build + `cargo tree` dep-absence assertion — CI-scriptable, automatable, though not itself a `--lib` test); `cargo test --lib` green. *Gate (on-machine):* live app opens an app, reads the tree, and drives one real approved Send — **integration/manual, not `cargo test --lib`.**

**Net testability:** slices 0, 2, 3, 4 (server half) are fully `cargo test --lib` with a mock backend — including the two gates the review called unmeetable (slice-3 "scroll, no prompt" and slice-4 "Session pins semantic target"), which are now achievable because scroll is `Safe` and the fingerprint is semantic-by-construction. Slices 1, 5, 6 carry honestly-flagged on-machine/UI gates that a `--lib` run cannot cover.

---

## STATUS (Revision v2)

**Build-ready** on the architecture. All three review gaps are resolved within the real spine — no `HookResult::Modify`, no fingerprint recompute, no parallel routing machinery; the semantic-args reframe makes the existing `ActionFingerprint`/`covers`/`covers_once`/`resolve_grant`/`enforce_local_routing` do the work, with two small *additive* dispatcher edits (both following the `protected_path` Once-pin and `ctx.routing` precedents) and one registry-exclusion requirement. The load-bearing invariants are intact and, for the irreversible leg, strictly-as-safe. Slice gates are honestly split into `cargo test --lib` (mock-backend) vs. flagged on-machine/UI integration.

**One open Lukas decision remains, and it is a policy toggle, not a blocker:** **OQ-1** — whether cloud vision is ever an opt-in (`Binding::Public` + a screenshot-specific consent) or screenshots are local-only forever. Fix 3 builds and ships the safe default (`image_in_window ⇒ LocalRequired`) today regardless; the OQ-1 answer only decides whether a future consent toggle is ever allowed to relax it. OQ-2 (reversibility taxonomy owner) and OQ-4 (capture-scope default) are confirms, not blockers, per the review. Proceed to build slice 0.
