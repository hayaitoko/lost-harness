...
```bash
cd /Users/hayai/Desktop/lost-harness-product/src-tauri && cargo test --lib   # expect: 564 passed, 0 failed
cd /Users/hayai/Desktop/lost-harness-product && npm run build               # expect: clean
cd /Users/hayai/Desktop/lost-harness-product && npm run check               # expect: 0 errors, 0 warnings
...
Optional env-gated live/model tests (not part of the 564; run manually):
```bash
...
Last verified: 2026-07-22 (M8 probe+calculator review: **564 passed**, `--no-default-features`
builds clean, `cargo clippy --lib` 0 errors, frontend build + svelte-check
clean (the then-known config warning was removed in the 2026-07-25 final pass), tree clean on `main` at `2adb022`).
