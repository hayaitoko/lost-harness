...
> Round 2 fixed the endpoint-routing bug Lukas hit in hands-on testing, made Google 403s recoverable,
> shipped app self-update, and corrected the provenance trust label. Four workstreams merged in order
...
> tag-triggered signed-release job, unbypassable signature verification, CI verifying the emitted
> signature against the pubkey the app ships, download host constrained; new doc
> [`releasing.md`](releasing.md). **D** — the provenance label now matches the `curated` wire value,
...
> update; (2) the updater's **end-to-end acceptance (a staged v0.1.0 → v0.1.1 update that downloads,
> installs and relaunches on the real app) was NEVER EXECUTED** — publishing was out of scope; the
> tampered/unsigned-artifact **refusal is** proven by tests; (3) the `.app` is **not** Apple-codesigned
...
> `f6535d4`; that is intentional, not a problem to fix).
>
...
# CI's real clippy gate (build.yml:151). NOTE: plain `-D warnings` fails with ~110 PRE-EXISTING
# errors in ~39 files no recent campaign touched — that is not the gate and not a regression.
...
| **M9** — polish | auto-update, signing, tray, Windows depth | 🟡 **Self-update BUILT 2026-07-30** (round 2 / C, `9dee37f`) — **the rest of M9 is not started.** `tauri-plugin-updater` (the repo's first Tauri plugins): non-blocking launch check, skipped on dev builds, backend-persisted toggle (default ON) that fails closed, never a silent install, Settings → About pane, a tag-triggered signed-release job extending `build.yml`, unbypassable signature verification against the real 2026-07-29 ceremony pubkey, CI verifying the emitted signature against the pubkey the app ships, download URL constrained to the expected GitHub release host. Doc: [`releasing.md`](releasing.md). **⚠ Unproven:** the end-to-end acceptance (staged v0.1.0 → v0.1.1 that downloads, installs and **relaunches** on the real app) was **never executed** — publishing was out of scope; tampered/unsigned **refusal is** proven by tests. **⚠ The `.app` is neither Apple-codesigned nor notarized** (the workflow says so itself) — an open Lukas decision that may affect whether an update installs on real macOS. Tray, Windows depth, and packaged classifier/embedder provisioning: **not started**. |
