...
> trust label. Four workstreams merged in order
> **A+D → B → C** with zero conflicts (`c5da00a` → `60025e9` → `9dee37f`, plus the follow-up de-flake
> `8b05bca`, on top of round 1's `0de478b`).
> **What each delivered:** **A** — the active model selection is now an indivisible `(provider, model)`
> pair that only an explicit user choice may set; no silent substitution anywhere; model-listing
> failures surface instead of hiding an endpoint; the composer fails closed rather than sending an
> empty provider; **PROFILE schema v12** stamps each message's trust zone at send time so a past turn's
> zone is a historical fact (unknown renders "Unknown", never a reassuring "Local"); the unusable
> "Anthropic" quick-add preset is removed. **B** — both Google 403 envelopes are classified
> structurally into two distinct states: scope-insufficient flips the reconnect banner, API-not-enabled
> gets its own state with the console link per (profile, API) and deliberately does **not** drive
> reconnect (reconnecting cannot enable a disabled API); unmatched 403s record nothing; the agent-tool
> path and `Planner.svelte` — neither of which the spec knew was broken — are fixed too. **C** —
> `tauri-plugin-updater` (the repo's first Tauri plugins): non-blocking launch check, skipped on dev
> builds, backend-persisted toggle that fails closed, never a silent install, Settings → About pane,
> tag-triggered signed-release job, unbypassable signature verification, download host constrained, and
> a CI step **written** (but never run — see "NOT done") to verify the emitted signature against the
> pubkey the app ships; new doc [`releasing.md`](releasing.md). **D** — the provenance label now matches the `curated` wire value,
> with a two-sided contract test so a future rename fails a test instead of inverting the trust badge.
> **Gates on the merged tree:** 1010 Rust lib tests (0 failed, 2 ignored, 4 consecutive clean runs),
> 230 frontend tests, clippy `-D clippy::correctness` 0 errors, svelte-check 223 files / 0 errors,
> `npm run build` green, `cargo fmt --check` clean. Baseline was 912 Rust / 115 frontend.
> **⚠ What is NOT done — say this out loud before anyone calls round 2 finished:** (1) **no live-app QA
> anywhere** — everything is verified by gates, unit tests and component tests driving the real Svelte
> components; nobody ran the built Tauri app against a real bad endpoint, a real Google 403, or a real
> update; (2) **the endpoint bug was never REPRODUCED** — A's root cause is the mechanism a read-only
> static investigation identified, strongly consistent with the symptom and closed by the fix, but not
> an observed cause; the spec's repro harness (a fake OpenAI-compatible endpoint on 127.0.0.1, asserted
> in the live app) was never built, so confirming the fix against Lukas's real setup is outstanding;
> (3) the updater's **end-to-end acceptance (a staged v0.1.0 → v0.1.1 update that downloads,
> installs and relaunches on the real app) was NEVER EXECUTED** — publishing was out of scope; the
> tampered/unsigned-artifact **refusal is** proven by tests; (4) **the `release:` CI job has never run
> at all**, so its signature-verification guard is written but **UNEXERCISED** — it fires only on a
> `v*` tag, no tag exists, and the job is present only on local unpushed `main` (`origin/main`'s
> `build.yml` has no `release:` job); (5) the `.app` is **not** Apple-codesigned
> or notarized (the workflow says so itself), which may affect whether an update installs on real
> macOS. Adversarial review refuted **every** workstream at least once (twice on A) — three blocking
> defects, all fixed and re-verified; detail in HANDOFF's 2026-07-30 entry and `PROGRESS-MAP-2.md`
> (untracked, on Lukas's disk — see HANDOFF for the convention).
> **Waiting on Lukas:** publish the first release · decide on Apple codesigning/notarization · review
> and push if he wants rounds 1+2 on `origin` (local `main` is deliberately ahead of `origin/main`
> `f6535d4`; that is intentional, not a problem to fix) · confirm the endpoint fix on his real setup,
> since the symptom was never reproduced.
>
> **✅ As of 2026-07-25: COMPLETION PASS.** The remaining advertised implementation seams are now closed: the model chooser derives downloadable artifacts and hashes from the current Hugging Face publisher manifest (no baked catalogue); Calendar and Tasks share the existing per-profile Google OAuth connection in the Planner and gated agent tools; MCP supports stdio and Streamable HTTP; and macOS computer-use resolves a fresh Accessibility target before Quartz synthesis. The prior audit fixes remain included (masked OAuth client secret, durable conversation binding, visible provider/chat write failure, stale-response guards, and removed dead UI). **Manual environment validation remains:** enable the three Google APIs and reconnect an existing Gmail-only grant; grant macOS Accessibility permission; test a real OAuth consent round and a real remote MCP server. Those are configuration/integration tests, not deferred code paths. Deliberate longer-term backlog is packaged classifier/embedder provisioning, onboarding, the server twin, and non-macOS native backends.
>
> **✅ As of 2026-07-24: UI BRIDGE CAMPAIGN COMPLETE — THE APP IS FUNCTIONALLY COMPLETE (685 tests).** Every backend feature is now reachable from the UI and no nav entry leads to dead/mock UI (`d61cdc3`→the campaign head; full detail in HANDOFF's 2026-07-24 entry): the reroute + budget toasts and the chat Stop button (C7 cancel) are live; Settings gains spend-cap / shell-network-ceiling / MCP-servers panes + the trusted-by-name notice + real version; **M8 S5 shipped** (interactive HF search + per-quant hardware calculator; download-from-search stays F7-gated); ScheduledJobs is live on a thin new cron IPC (+contract test); Files is a real Tier-P-confined read-only workspace browser (+`list_workspace_files` IPC +contract test); the Email/Whiteboard/Editor/EmptyState/Onboarding mockups and 16 never-imported components are deleted. Adversarially reviewed (find→verify, 4 lenses). **What "functionally complete" excludes (tracked, deliberate):** email/calendar (own round, needs M7-Q2), M5 native computer-use backends, MCP SSE/HTTP, model download-from-search (F7 attestation gate), onboarding/first-run flow, Wave 6 server twin, other OSes. **Next: Lukas's hands-on user-testing pass.**
>
> **SCOPE DECISION (Lukas, 2026-07-22): the "BACKEND-FINISHED" line.** Three rounds to a finished macOS backend, then the UI/usability/onboarding phase: **Round A** finish M8 backend (S2′+S4+IPC exposure+probe cache+live E2E); **Round B** the hardening round (all 2026-07-21/22 audit debt); **Round C** feature-complete: budget governor, durability journal, MCP wire transport, skill-as-Tool wrapper, reroute backend half, M5 logic half (mock backend), M6 cancel plumbing. **IN-SCOPE decisions:** MCP transport IN; skill-as-Tool IN. **DEFERRED post-UI:** teacher-escalation + curator rot-check (behavioral polish, judge after real use). **CUT until a real trigger exists:** capability registry 3.4. **DEFERRED to its own round:** email/calendar/tasks (needs the M7-Q2 backend decision + creds). **OUT of "finished":** Wave 6 server twin (separate campaign), other OSes (own later campaign), S5/UI (the next phase, Lukas's eyes on it).
>
> **✅ As of 2026-07-23: THE macOS BACKEND + EXTERNAL-REVIEW FIX BATCH ARE COMPLETE (683 tests).** All three build rounds, the two review-found frontend fixes, and Lukas's 2026-07-23 security decisions are implemented and gate-green. The latest batch adds OS credential-store-backed provider keys with a failure-safe legacy migration; tool-destination-aware privacy gating (including the Remote MCP risk floor); MCP child env scrubbing; live 7/30/90-day retention with redacted tool-audit args; name-trust metadata; full bundle icons; release-blocking clippy correctness; advisory RustSec audit; and frontend type-check CI. A real debug Tauri bundle produces both `Lost Harness.app` and its DMG. Public remains a pure user waiver and the rules-only classifier fallback remains permissive by explicit decision. **The next phase is UI / usability / onboarding (Lukas's eyes on it).** Top items remain the `stream:local_reroute` and `stream:budget_warning` consumers, the name-trust/model-search notices, and the documented native/transport/provisioning gaps in HANDOFF. **Rounds A+B+C recap:**
>
> **As of 2026-07-22: ROUNDS A+B COMPLETE — was on Round C (feature-complete) (632 tests).** Round B (the hardening round) closed all ten 2026-07-21/22 audit-debt items (`8f2712a`→`b727bec`), each cited to its finding, gate-green + a 3-lens adversarial review: the seat-routing regression test, profile-name validation, `complete()`-path usage booking + crash audit row, per-profile tool-action gating + the cron-replay guard, the real-loop fake-streamer harness, the guard-wrap + work-runner-supervisor tests, `sandbox_config` IPC (the Tier-K ceiling is reachable), full-surface contract coverage, and the content-free `stt_egress` + one-confirm TTS. See the HANDOFF "ROUND B COMPLETE" entry. **Round A recap:**
>
> **ROUND A COMPLETE — the M8 backend is FINISHED + LIVE-VERIFIED (612 tests).** All five items landed in order, each gate-green + adversarially reviewed: A1 HF search + GGUF metadata reader (`fe258cd`), A2 the bundled llama.cpp `b10088` sidecar + supervisor + lazy spawn (`4afb25a`), A3+A4 the model IPC surface + boot-cached probe (`d3137dd`), A5 the full live E2E (`d8ae711`). The env-gated live tests (`LHP_HF_LIVE`, `LHP_E2E_LIVE`) both ran GREEN against real Qwen3-0.6B bytes (search→download→verify→spawn→chat→teardown, 52.9s) — and A5 earned its keep by catching a real live-API bug the fixtures missed (HF sends both `id` and `modelId`). **Now on Round B (the hardening round).** History of the pure-brains work below:**
>
> **Earlier 2026-07-22: the two pure BRAINS built (564 tests).** Phase-1 design revision + Lukas's product redirect (HF search + interactive calculator, NOT a static catalog — see the M8 milestone row + "REVISION 2026-07-22/22b" in the m8 plan doc), then **S1 probe v2** (`1c8bca0`: bandwidth-by-chip-family / GPU enum / unified-memory, live-verified on the M3 Max) and **S3′ the calculator engine** (`ac84d25`: pure fit + tok/s as f(weight quant, KV quant, context), MoE-aware, honest-Unknown, refuse-loudly). Remaining M8: S2′ HF search + GGUF metadata reader → S4 sidecar (llama.cpp b10088 bundle + supervision) → S5 the interactive search+calculator screen + BYO-GGUF + external-endpoint path → the env-gated live E2E. Independent re-review of S1+S3′ ran 2026-07-22 (this session).
>
...
```bash
cd /Users/hayai/Desktop/lost-harness-product/src-tauri && cargo test --lib   # expect: 1010 passed, 0 failed, 2 ignored
cd /Users/hayai/Desktop/lost-harness-product && npx vitest run              # expect: 230 passed, 16 files
cd /Users/hayai/Desktop/lost-harness-product && npm run build               # expect: clean
cd /Users/hayai/Desktop/lost-harness-product && npm run check               # expect: 223 files, 0 errors, 0 warnings
# CI's real clippy gate (build.yml:157). NOTE: plain `-D warnings` FAILS — 110 errors (lib) + 77
# (lib test), i.e. 128 distinct diagnostics at 127 locations across 39 files. That is not the gate.
# 12 of those 39 files WERE edited by round 2 (the errors sit on lines it did not write); round 2
# also introduced 2 of its own, found by a doc audit and fixed in c1b9fae. After c1b9fae every one
# of the 127 locations was blamed against 0de478b..HEAD: 0 attributable to round 2. See HANDOFF.
cd .../src-tauri && cargo clippy --all-targets -- -D clippy::correctness    # expect: 0 errors
...
Optional env-gated live/model tests (not part of the 1010; run manually):
```bash
...
Last verified: **2026-07-30** (round-2 merge, `main` at `8b05bca`): **1010 passed / 0 failed / 2
ignored** (4 consecutive clean runs), **230** frontend tests over 16 files, `cargo clippy
--all-targets -- -D clippy::correctness` 0 errors, `cargo fmt --check` clean, `npm run check` 223
files / 0 errors / 0 warnings, `npm run build` green. Schemas: GLOBAL **v9**, PROFILE **v12**. The 2
ignored tests are one pre-existing and one added by round-2 workstream C
(`a_real_bundler_produced_artifact_verifies`, needs a local `tauri build`). **NOT re-run on this
tree** — `cargo audit`, `cargo build --lib --no-default-features`, `npm audit` (all clean as of
round 1; unverified today). A pre-existing flake
(`models::download::tests::an_overlong_stream_is_refused…`) is still in the tree; it did not fire
across the round-2 runs. _(Previous entry: 2026-07-22, 564 passed, tree clean at `2adb022`.)_
...
| **M4** — model manager + skills/agents | native tool-use, seats, usage ledger, budget governor, cache-shaped prompts; skills & agents track | 🔵 **In progress** — Q8 (grant×risk matrix + persisted `tool_rules` + risk-badged dialog) done 2026-07-16. **Native tool-use (Q1) DONE + PROVEN LIVE 2026-07-17** (`d203a9a`): per-endpoint `supports_native_tools` flag, structured `tool_calls` transport + fenced fallback, one transport-blind pipeline, fingerprint parity tested, and verified end-to-end against LM Studio qwen3.6-35b-a3b (3 clean runs). **Usage ledger (3.2) booking side DONE 2026-07-17** (`7141d34`): per-profile `usage_events` cost ledger, every model call booked, local=$0/cloud=unknown-flagged. **Cache-shaped prompts + context compaction (3.3) DONE 2026-07-17** (`f543250`): byte-stable prefix + deterministic `compact_history` at the stream seam, emits the 3.5 pre-compaction signal (+ a HIGH cloud-history privacy fix `c026e33`). **Model seats (3.1) DONE 2026-07-18** (`ec6c852`, per-profile, user-definable strings, unbound→inherit). **Real cost capture DONE 2026-07-18** (`649f3fa`). **Wave 3.5 flush DONE.** **The whole skills & agents track (Wave 4.1–4.5) DONE 2026-07-18** — skills, learning loop, agent registry + `delegate` runtime, one-queue + cron runner, capability packs. **Budget governor DONE 2026-07-23** (C1, `4a33931`): per-profile USD cap (schema v11), unattended work HALTS before the model call, attended chat WARNS (non-blocking), fail-closed on unknown-cost/read-error; get/set/reset IPC. **Skill-as-Tool DONE 2026-07-23** (C4, `f318c0c`). **Still open in M4 (all now UI/post-backend):** capability registry that refuses (3.4 — CUT until a real trigger exists, per the scope decision), reroute auto-switch UX (2.3 — backend half DONE in C5 `7cb0d5a`, the toast/switch is the UI phase). ✅ 2026-07-21 audit item CLOSED: the end-to-end "a cloud seat can't defeat RouteLocal" regression test was written in Round B (B1). Budget hard-cap-via-cost-reservation is on the deferred ledger. **PROVIDER ENDPOINT ROUTING FIXED 2026-07-30** (round 2 / A, `c5da00a`) — the user-observed "sent to Anthropic instead of my OpenAI endpoint" bug. The mechanism identified was **frontend selection state**, not the manager: `hydrateProviders` silently armed `merged[0]`, which is the ALPHABETICALLY-FIRST provider (endpoints list `ORDER BY name`, `storage/global.rs:606`), and "Anthropic" sorted first among the quick-add presets; `fetchModels` swallowed listing failures so the user's real endpoint vanished from the picker. Now the selection is an indivisible `(provider, model)` pair only an explicit user choice may set, listing failures surface, the composer fails closed, **PROFILE v12** stamps `messages.endpoint_zone` at send time (unknown → "Unknown", never "Local"), and the unusable "Anthropic" preset is removed. **⚠ Not live-QA'd, and the symptom was never REPRODUCED** — the chain above comes from a read-only static investigation; it is strongly consistent with what Lukas reported and the fix closes it, but it is not an observed cause. Verified by gates + component tests, not by sending a turn from the built app; the spec's live repro harness was never built. Confirming against Lukas's real setup is outstanding. |
...
| **M5** — computer use | cross-platform screen control, the flagship | 🟡 **Logic half WIRED 2026-07-23 (C6, `f318c0c`); native backends remain target work.** The reversibility classifier is now LIVE behind the `ui_*` act tools (`tools/computer_tools.rs` — semantic app/role/label locators only, Scroll=Safe/actuation=External) over the `ComputerBackend` seam (`tools/computer_backend.rs` — mock + refuse-loudly `UnavailableBackend` in production), gated by the `OnScreenActionHook` (`hooks/on_screen_action.rs` — fresh-snapshot re-resolve → Deny on moved/vanished; `covers_once` Once-only floor for IRREVERSIBLE labels). All behind the default-on `computer-use` feature, fully `cargo test --lib`-proven against the mock. **Still target work:** the native capture/AX backend (macOS ScreenCaptureKit/AX + CGEvent synthesis), the capture/read tools (`ui_tree`, screenshot), the multimodal wire format's live callers, Win/Linux. Audit 2026-07-21: cores match their design. |
| **M6** — voice | on-device STT/TTS, barge-in | 🟡 **Audio-egress gate + cooperative cancel built; native voice on-target.** ✅ Both 2026-07-21 audit deviations FIXED in Round B (B9, `b727bec`): `stt_egress` is now the content-free pre-transcription decision, and Public+floor uses one confirm via the approval spine (`ConfirmRequired`, single-use `take_once`). **Cancel plumbing (Slice 4a) DONE 2026-07-23** (C7, `71c0a99`): a `CancellationToken` through the real `process_message` — cooperative SSE drain break, `aborted:true` persisted, `cancel_message` IPC. **Still target work:** native STT/TTS/AEC, barge-in, the ConfirmRequired UI round-trip. |
| **M7** — per-profile isolation | email/calendar/tasks, Capability Packs, real OS sandbox enforcement | 🟡 **Tier-P (per-profile fs confinement) COMPLETE + LIVE** (`3801a0e` — strongest-evidenced area of the 2026-07-21 audit). Capability Packs shipped with Wave 4.5. **Tier-K partial:** the macOS shell-network ceiling enforcement is live code (`tools/exec.rs::effective_network`) **but unreachable in practice — no IPC/UI ever writes a `sandbox_config` row**, so every real profile takes the unconfigured branch today; needs a small settings surface (headless-buildable). Linux/Windows sandbox backends on-target. **Email/Calendar/Tasks are live** on the per-profile Google OAuth connection, with Planner IPC/UI and gated agent tools. **403 RECOVERY ADDED 2026-07-30** (round 2 / B, `60025e9`): both Google error envelopes are classified structurally into two distinct states — scope-insufficient flips the existing reconnect banner; API-not-enabled gets its own state with the console activation link, keyed per (profile, API), and deliberately does **not** drive reconnect (reconnecting cannot enable a disabled API). Unmatched 403s record nothing; the classification travels as typed data, never re-parsed from an error string; console links are sanitised (https only, console.cloud/console.developers hosts, no userinfo, no non-default port). Two gaps the spec didn't know about were fixed too: `tools/email.rs` (the agent-tool path) duplicated the marker-only logic, and `Planner.svelte` never read connection state at all. **⚠ Not live-QA'd** — no real Google 403 was ever driven through the built app. |
| **M8** — settings/onboarding/hardware | hardware probing, model catalog, first-run | 🟢 **BACKEND COMPLETE + LIVE-VERIFIED (Round A, 2026-07-22).** HF model search (`hf_search.rs`) + GGUF metadata reader (`gguf_meta.rs`) + the bundled llama.cpp `b10088` sidecar + supervisor + lazy `ensure_running` + boot integrity sweep + the `search_models`/`get_model_detail`/`calculate_model_fit` IPC + a boot-cached probe — all committed (`fe258cd`→`d8ae711`), 612 tests, and the full search→download→verify→spawn→chat→teardown chain runs GREEN live (`LHP_E2E_LIVE=1`). Only S5 (the interactive search+calculator UI) remains — the UI phase. _History below:_ 🟡 **Substantially built + LIVE in Settings→Models:** hardware probe (fails closed on unknown RAM), curated catalog, download + SHA-verify + resume (HF-only host allowlist), model list/remove. **Open:** catalog ships `sha256="TODO-CURATE"` placeholders — fails closed, so NOTHING is installable until real hashes are curated (headless-buildable); S4 `llama-server` sidecar (native, needs the C9/M8-Q2 Lukas decision); boot-time integrity re-check primitive exists but is unwired. Onboarding screen still visual-only. **DECIDED 2026-07-21 (Lukas): bundled sidecar = YES** — the app must be able to run its own models for users without external infra; the hardware probe must make an EDUCATED choice (memory bandwidth + dense-vs-MoE + GPU count/topology + quant, not just RAM capacity); external OpenAI-compatible endpoints (Lukas's own use case) remain a first-class, equally-easy path. macOS/Metal sidecar first; Win/Linux sidecar backends ride Wave 7.4. **PHASE 1 DESIGN REVISION LANDED 2026-07-22** (docs/plans/2026-07-18-m8-model-lifecycle-design.md, "REVISION 2026-07-22"): probe v2 (bandwidth/GPU/unified-memory + honest-Option), a pure recommendation engine (MoE-vs-dense/bandwidth/quant/refuse-loudly), catalog v2 (per-quant artifacts + arch + quality tier + the sha256-via-HF-API cheap-curation mechanism), and the sidecar (llama.cpp `b10088` bundle-at-build via `bundle.resources` — NOT externalBin, 9 dylibs; supervision + lazy `ensure_running` + boot integrity sweep wiring the callerless `set_model_status`) — all 4 reconciled onto ONE shared data-model contract (§0) and skeptically reviewed (4 blocking data-model mismatches resolved). Build slices spec'd. **S1 (probe v2) BUILT + committed `1c8bca0`** (bandwidth-by-chip-family + GPU enum + unified-memory + fake-probe trait; real-hardware-verified; 1 review bug fixed; 553 tests green). **PRODUCT REDIRECT 2026-07-22b (Lukas + LM Studio screenshot):** the discovery half is re-scoped from a curated bundled catalog to a **HuggingFace model SEARCH (like LM Studio) + an interactive hardware CALCULATOR** (fit + tok/s for the user's machine as a function of weight quant, KV-cache quant, and context size — live). Curated list → "Staff picks" seed; sha256 comes from HF's tree API at download time (verified-before-runnable unchanged). Revised slices: **S1 done (`1c8bca0`) → S3′ calculator engine done (`ac84d25`, `models/calculator.rs`: KV/weights/overhead/TPS/fit, MoE-aware, fail-closed+honest-Unknown, 11 tests) → [next] S2′ HF search + GGUF metadata reader → S4 sidecar (llama.cpp b10088 bundle + supervision) → S5 interactive search+calculator UI + BYO-GGUF + external-endpoint path → env-gated live E2E test.** The two pure "brains" (probe + calculator) are built + reviewed + green (564 tests). Design in "REVISION 2026-07-22" + "2026-07-22b" of the m8 plan doc. **Independent re-review 2026-07-22c: SOLID; fixed 2 mis-asserting calculator tests + added the HF trust-root honesty note/S2′ trusted-publisher requirement to the design doc; calculator confirmed pure + zero live callers (awaits S5); probe fields not yet in the TS type/UI (S5); probe uncached per call (known TODO).** **PROVENANCE LABEL CORRECTED 2026-07-30** (round 2 / D, `c5da00a`): the frontend declared `"trusted" | "community"` while the backend `Provenance::Curated` serialises snake_case `"curated"`, so the trust badge was inverted at the wire. Fixed to the real wire value + a two-sided contract test so a future rename fails a test instead of silently flipping the badge. Invisible today (no signing key is compiled in, so nothing is ever `Curated`) — it would have surfaced the moment the manifest ceremony lands. |
| **M9** — polish | auto-update, signing, tray, Windows depth | 🟡 **Self-update BUILT 2026-07-30** (round 2 / C, `9dee37f`) — **the rest of M9 is not started.** `tauri-plugin-updater` (the repo's first Tauri plugins): non-blocking launch check, skipped on dev builds, backend-persisted toggle (default ON) that fails closed, never a silent install, Settings → About pane, a tag-triggered signed-release job extending `build.yml`, unbypassable signature verification against the real 2026-07-29 ceremony pubkey, download URL constrained to the expected GitHub release host, and a CI step written to verify the emitted signature against the pubkey the app ships. Doc: [`releasing.md`](releasing.md). **⚠ Unproven:** the end-to-end acceptance (staged v0.1.0 → v0.1.1 that downloads, installs and **relaunches** on the real app) was **never executed** — publishing was out of scope; tampered/unsigned **refusal is** proven by tests. **⚠ The whole `release:` CI job — signing, the `minisign -V` guard, the upload — has NEVER RUN:** it fires only on a `v*` tag, no tag exists locally or on `origin`, and the job is present only on local unpushed `main` (`git show origin/main:.github/workflows/build.yml` has no `release:` job). The guard is written, not exercised. **⚠ The `.app` is neither Apple-codesigned nor notarized** (the workflow says so itself) — an open Lukas decision that may affect whether an update installs on real macOS. Tray, Windows depth, and packaged classifier/embedder provisioning: **not started**. |
...
> **CURRENT LIST (2026-07-30, after round 2).** The top of the queue is **Lukas's**, not an
> engineering item:
>
> 1. **Lukas: publish the first release.** This is the only way to prove the end-to-end self-update
>    (a staged v0.1.0 → v0.1.1 that downloads, installs and relaunches on the real app). It was never
>    executed — publishing was out of scope for round 2. [`releasing.md`](releasing.md) has the exact
>    steps.
> 2. **Lukas: decide on Apple codesigning/notarization.** The `.app` is neither today. Worth settling
>    before cutting a release people rely on, because it may decide whether an update installs at all
>    on real macOS.
> 3. **Lukas: review and push**, if he wants rounds 1 + 2 on `origin`. Local `main` is deliberately
>    ahead of `origin/main` (`f6535d4`).
> 4. **Live-app QA of round 2 — nobody has done any.** All three fixes are verified by gates, unit
>    tests and component tests driving the real Svelte components; none has been exercised in the
>    built Tauri app. Concretely: (a) stand up a fake OpenAI-compatible endpoint on 127.0.0.1, add it
>    as a provider in the live app, send a turn and assert it lands there (the spec's own repro
>    harness, meant to become the permanent live check); (b) drive a real Google 403 — both the
>    scope-insufficient and API-not-enabled shapes — through the live Planner/Email screens; (c) run a
>    real update once a release exists.
> 5. Then the standing backlog below (nothing in it changed in round 2): packaged classifier/embedder
>    provisioning, onboarding/first-run, M5 native computer-use backends, non-macOS backends, the
>    Wave 6 server twin.
>
> **✅ THE 2026-07-21 LIST BELOW IS FULLY DRAINED (2026-07-23).** Every item below (1–9) landed across Rounds B and C — items 1–8 in Round B's hardening pass (`8f2712a`→`b727bec`, see the HANDOFF "ROUND B COMPLETE" entry), item 9's four fronts in Round C (MCP wire transport `f8cb422`, durability journal `f318c0c`, M5 logic half `f318c0c`, M6 cancel `71c0a99`). **The macOS backend is FINISHED.** The next backlog is the UI/onboarding phase — its starting point is the "DISCOVERED-BUT-DEFERRED" ledger in HANDOFF.md. The list is kept below for the record.
>
> **HISTORICAL CURRENT LIST (2026-07-21, from the full project review — supersedes the numbered
...
- **Waiting on Lukas (2026-07-30, from round 2):**
  - **Proving the self-update end-to-end is blocked on publishing the first release.** Nothing in
    the code is missing; there is simply no release to update *from* or *to*. Steps:
    [`releasing.md`](releasing.md).
  - **Apple codesigning / notarization is an open decision.** The `.app` is neither. This may
    determine whether an update actually installs on real macOS.
  - **Pushing rounds 1 + 2 to `origin`** is his call. Local `main` (`8b05bca`) is deliberately ahead
    of `origin/main` (`f6535d4`). Do not push without him.
- (The native-tool-use live proof — previously blocked on LM Studio's
...
- Frontend type-check is clean (the former duplicate-config warning was removed in the 2026-07-25 final pass).
