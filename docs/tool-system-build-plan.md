...
| 3 | Protected-paths floor hook | done | d13d71a | New `hooks/protected_path.rs` + `ProtectedPathHook` (no config, no `PolicySource`/constructor args — the hardcoded list IS the invariant). Sits between `SandboxHook` and `PermissionHook` in all three chain constructors (`_with_confirmed` uses bare `ProtectedPathHook::new()`, `_full` uses `with_ledger(Arc::clone(&ledger))`). `ApprovalLedger::covers_once` added (only inspects `once_fps`); `ProtectedPathHook` consults ONLY that method, so a `Session`/`Tool` grant is invisible. `tools/dispatch.rs` Approve arm pins a forced `Once`+`Fingerprint` grant when the user answers a protected-path prompt with anything broader than `Once` (so the re-run settles, but the floor itself stays Once-only). 11 new tests (7 in protected_path.rs, 1 covers_once in approval.rs, 1 chain order + 1 floor-runs-before-permission in tests.rs, 2 dispatcher integration tests). 245 total green. `docs/codebase/hooks-gating-and-approval.md` updated to reflect the 5-hook chain. `cargo build` still 75 warnings (pre-existing). Next agent: Item 4 (crash-recovery + tool.interrupted). |
| 5 | tool_audit + PostToolUse observer | done | f72a7f9 | New `tool_audit` table in per-profile DB (migration v2, `PROFILE_SCHEMA_VERSION` bumped from 1 to 2; `GLOBAL_SCHEMA_VERSION` stays at 1, so the migration runner's debug_assert was split into two consts). New `hooks/audit.rs` with `AuditEntry`, `AuditWriter` trait, `StorageAuditWriter` (opens per-profile DB on each call, never bubbles write errors), `AuditObserverHook` (sync `on_event` is a no-op by design — outcome is in scope at the dispatcher's call site, not in `EventContext`), `outcome_label` / `outcome_gate_by` mappers, `truncate_args` (4KB cap, char-boundary safe). New `ToolAuditRow` + `add_tool_audit` + `list_tool_audit` on `ProfileDb` (append-only: no UPDATE / DELETE methods). `ToolDispatcher` gains `audit_writer: Option<Arc<dyn AuditWriter>>` + `with_audit_writer` builder + private `fire_audit` helper. `dispatch` is now a thin wrapper: `start = Instant::now()`; `outcome = dispatch_inner(...).await`; `fire_audit(...)`; `outcome` — the inner method returns `ToolOutcome` directly, so the audit fires exactly once per call. `EventContext::post_tool_use()` constructor added to `hooks/mod.rs` for the future `notify_observers` migration. `lib.rs::build_tool_dispatcher` takes a new `storage: Storage` param, wires `StorageAuditWriter` into the dispatcher, and registers `AuditObserverHook` in the chain for structural completeness. `grant_used` + `decision` audit fields are `None` for now — derivable in a later round when the dispatcher's for-loop can thread them out. 5 new dispatcher tests (denied/ok/unknown/unavailable/no-writer) + 3 storage round-trip tests. 258 total green, 76 warnings (all pre-existing). Next agent: Item 4 (crash-recovery + tool.interrupted). |
